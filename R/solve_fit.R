...
set_edge = function(f) {
  ...
  u_vals = attr(ignr, "values")
  u_valx = u_vals[!is.na(u_vals)]
  n_valx = matrix(t(sapply(u_valx, function(h) colSums(f_edge == h, na.rm = TRUE))), ncol = d[3])
  dimnames(n_valx) = list(u_valx, dimnames(f)[[3]])
  pcol_vals = head(u_vals, ifelse(length(u_vals) == 1, 1, -1))

  d_pcol = mbindlist(setNames(lapply(pcol_vals, function(val) {
    f_edge[is_diag, , drop = FALSE] == val & !ignr[is_diag, , drop = FALSE]
  }), pcol_vals))
  o_pcol = mbindlist(setNames(lapply(pcol_vals, function(val) {
    f_edge[!is_diag, , drop = FALSE] == val & !ignr[!is_diag, , drop = FALSE]
  }), pcol_vals))
...
set_vars = function(w, edge, vertex, FUN) {
  coef = list(
    edge_coef = t(matrix(
      data = kronecker(w[t(edge$id)], edge$values, FUN),
      nrow = length(edge$values),
      dimnames = list(edge$values, colnames(edge$id))
    )),
...
edge_coef_constraints = function(constr, vars, edge) {
  for (h in seq_along(edge$values)) {
    ...
    constr$add_constraint(
      coef    = c(rep(1, edge$n),-edge$nvalues[h, ]),
      indices = c(vars$posn$edge_coef[, h],vars$posn$config),
      dir     = "==",
      rhs     = 0
    )
  }
...
f_ignore = function(x, decreasing = TRUE) {
  u  = sort(
    unique(c(x)),
  ...
  if (length(u) < 2) {
    y = matrix(FALSE, nrow(x), ncol(x))
    attr(y, "ignore") = vector("numeric")
  } else {
    h = tail(u, 1)
    if (is.na(h)) {
      y = is.na(x)
