...
solve_fit = function(x, ...) {
  lp_solve_fit(x[[1]], x[[2]], ...)
}

#' @import ROI ROI.plugin.glpk Rglpk
lp_solve_fit = function(w, f, solver = "glpk", ...) {
  f1       = as.configuration(f)
  if (attr(f1, "type") == "binary") {
    f1[f1 == 0] = -1
  }
  FUN      = ifelse(attr(f1, "type") == "binary", prodNA, absdiffNA)
  maximize = attr(f1, "type") == "binary"
  edge     = set_edge(f1)
  vertex   = set_vertex(f1)
...
potential_score = function(w, edge, FUN, maximize) {
  edge_coef = matrix(kronecker(edge$values, w[t(edge$id)], FUN), edge$n)
